...
class MILSwinV2BaseClassifier(nn.Module):
    """BigLunge MIL classifier: attention pooling over whole-slice bags,
    SwinV2-Base (swinv2_base_window8_256.ms_in1k) backbone.

    Counterpart to ``MILSwinTinyClassifier``, using SwinV2-Base (~88M params,
    1024-d pooled features) instead of Swin-Tiny + RadImageNet weights.
    timm automatically adapts the 3-channel pretrained stem to 1-channel when
    ``in_chans=1, pretrained=True`` are combined. Input slices must be 256×256.

    DAPT is run with ``SwinV2Base2DClassifier`` (model_type ``swinv2_base_2d``).
    ``load_backbone_from_dapt`` accepts that checkpoint's state dict (keys
    prefixed ``swin.``) and loads the backbone weights non-strict.
    """

    BACKBONE_NUM_FEATURES = 1024  # SwinV2-Base stage-4 global-pooled dim
    ...
    def __init__(
        ...
        if not self._use_advanced_fpn:
            ...
            if mil_mode not in ("att", "att_trans"):
                print(f"[MIL-SwinV2] mil_mode={mil_mode} ignored in advanced FPN mode; using attention pooling.")
            self.backbone = timm.create_model(
                "swinv2_base_window8_256.ms_in1k",
                pretrained=True,
                features_only=True,
    ...
    def load_backbone_from_dapt(self, state_dict, logger=None):
        """Load a DAPT ``SwinV2Base2DClassifier`` state dict into the backbone.

        DAPT keys have the form ``swin.<timm_key>`` (the 2D classifier stores
        the timm model as ``self.swin``). Strips the prefix and loads non-strict;
        the DAPT 3-class head is dropped.
        """
        src_prefix = "swin."
        ...
        if logger is not None:
            logger.info(
                f"[MIL-SwinV2] Loaded DAPT backbone: matched {matched}/{len(target_sd)} target keys "
                f"(source contributed {len(mapped)} entries, missing={len(missing)}, "
                f"unexpected={len(unexpected)})"
            )
        return matched, missing, unexpected
    ...
    def forward(self, x, return_segmentation: bool = False):
        if x.ndim != 5:
            raise ValueError(
                f"MILSwinV2BaseClassifier expects (B, N, C, H, W); got shape {tuple(x.shape)}"
            )
        if not self._use_advanced_fpn:
            cls_logits = self.mil(x)
            if return_segmentation:
...
class MILSwinV2TinyClassifier(nn.Module):
    ...
    def load_backbone_from_dapt(self, state_dict, logger=None):
        ...
        DAPT keys have the form ``swin.<timm_key>``. Strips the prefix and
        loads non-strict; the DAPT 3-class head is dropped.
        """
        src_prefix = "swin."
        target = self.backbone if self._use_advanced_fpn else self.mil.net
        target_sd = target.state_dict()

...
class ModelRuntime:
    ...
    def _extract_sclc_outputs(self, output: Any, class_labels: list[str]) -> dict[str, Any]:
        ...
        if box_pred is not None:
            if box_pred.ndim == 3: # (B, N, 4)
                bbox_np = box_pred[0].mean(dim=0).cpu().numpy() # (4,)
            elif box_pred.ndim == 2: # (B, 4)
        ...
        return {
            ...
            "reasoning":          "Prediction generated by SCLC-Diagnostic multi-head model (classification + segmentation + detection).",
        }
    ...
    def compute_gradcam(self, class_idx: int) -> tuple[np.ndarray, np.ndarray] | None:
        """Return (cam_np (N,H,W), att_np (N,)) or None if prerequisites are missing."""
